...
WHERE Proposal_Code = %(proposal_code)s
    ...
    raise ValueError("Proposal text Could not be found.")


async def get_proposal_investigators(
    proposal_code: str, db: connect
) -> List[Dict[str, Any]]:
    sql = """\
SELECT pi.Investigator_Id, FirstName, Surname, Partner_Name, InstituteName_Name,
        Department, Url, Leader_Id, Contact_Id FROM ProposalInvestigator AS pi
...
async def get_proposal_allocations(
    proposal_code: str, semester: Semester, db: connect
) -> List[Dict[str, Any]]:
    sql = """
...
async def get_proposal_targets(proposal_code: str, db: connect) -> List[Dict[str, Any]]:
    sql = """
SELECT Target_Name, RaH, RaM, RaS, DecSign, DecD, DecM, DecS, Equinox, MinMag, MaxMag,
    ...
    MoonProbability, CompetitionProbability, ObservabilityProbability, SeeingProbability
...
async def get_observed_targets(
    proposal_code: str, db: connect
) -> List[Dict[str, Union[int, str, Date]]]:
...
async def get_proposal_requested_time(
    proposal_code: str, db: connect
) -> List[Dict[str, Any]]:
    requested_time_sql = """
SELECT TotalReqTime, P1MinimumUsefulTime, P1TimeComment, Year, Semester
FROM Proposal as p
            ...
            for r in drs:
                if not (r[2] == "OTH"):
                    ...
                    requested_time[_sem]["distribution"].append(
                        dict(
                            partner_code=r[2], partner_name=r[3], share_percentage=r[4]
                        )
                    )
            _req = []
